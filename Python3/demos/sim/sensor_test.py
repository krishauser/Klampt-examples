		...
		try:
			...
			if self.compute_pc:
				...
				if GET_POINTS_FROM_IMAGES:
					...
					self.rgb,self.depth = processDepthSensor(sensor)
					self.rgb[:,:,0] = 0
					self.pc = sensing.image_to_points(self.depth,self.rgb,xfov=float(sensor.getSetting('xfov')),yfov=float(sensor.getSetting('yfov')),points_format='Geometry3D',all_points=True)
