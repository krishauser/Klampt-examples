...
class Hand:
    ...
    def __init__(self,hand='l'):
        ...
        if hand[0] == 'l':
            ...
            self.localPosition1 = (-0.03,0.02,-0.1)
            self.localPosition2 = (0.03,0.02,-0.1)
        self.armIndices = list(range(self.link-5,self.link+1))
        self.fingerIndices = list(range(self.link+1,self.link+16))
    ...
    def keyboardfunc(self,key,x,y):
        if key == 'l':
            ...
            if self.path:
                ...
                self.path = RobotTrajectory(self.robot,list(range(len(self.path))),self.path)
            #reset the animation
    ...
    def keyboardfunc(self,key,x,y):
        ...
        if key in targets:
            ...
            if self.transitPath and self.transferPath and self.retractPath:
                ...
                self.path = RobotTrajectory(self.robot,list(range(len(milestones))),milestones)

                resolution = 0.05
                xfertraj = RobotTrajectory(self.robot,list(range(len(self.transferPath))),self.transferPath)
