...
a_pc.setCurrentTransform(so3.identity(),[1,0,0])
value = None
cmap = None
feature = None
...
def convert(value_new,cmap_new,feature_new=None):
    global value,cmap,feature,w
    ...
    if cmap is None:
        cmap = 'viridis'
    a_app = colorize.colorize(w.rigidObject(0),value,cmap,feature)
    #a_app.drawGL(a)
    if not value.startswith('n'):
        colorize.colorize(a_pc,value,cmap)
    vis.remove("A")
    vis.add("A",a_app)
    vis.remove("B")
    vis.add("B",a_pc)
